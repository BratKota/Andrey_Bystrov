...
def main():
    ...
    deviation_threshold = 488  # Задайте значение отклонения, с которым вы хотите фильтровать
    filter_by_deviation(df, deviation_threshold)
    # Использование функции для фильтрации по датам
    start_date = '26.08.2022'  # Задайте начальную дату
    end_date = '12.11.2022'    # Задайте конечную дату
    filter_by_date(df, start_date, end_date)
    # Группировка по месяцам
    group_month(df)
    # Построение графика для всего периода
    all_period(df)
    # Пример использования функции для отрисовки по месяцу
    target_month_year = '2022.02'  # Замените на нужный месяц с годом
